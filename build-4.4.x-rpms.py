...
yum install git ant ant-devel java-1.7.0-openjdk java-1.7.0-openjdk-devel mysql mysql-server tomcat6 mkisofs gcc python MySQL-python openssh-clients wget rpm-build ws-commons-util net-snmp genisoimage createrepo -y \n
mkdir -p public/cloudstack.apt-get.eu/rhel/4.4 \n
...
source ~/.bashrc \n
wget http://apache.osuosl.org/cloudstack/releases/4.4.2/apache-cloudstack-4.4.2-src.tar.bz2 \n
tar -jxvf apache-cloudstack-4.4.2-src.tar.bz2 \n
#cd apache-cloudstack-4.4.2-src \n
#rm -f ./server/test/org/apache/cloudstack/network/lb/CertServiceTest.java \n
#rm -f ./dist/rpmbuild/BUILD/cloudstack-4.4.2-SNAPSHOT/server/test/org/apache/cloudstack/network/lb/CertServiceTest.java \n
#rm -f ./dist/rpmbuild/SOURCES/cloudstack-4.4.2-SNAPSHOT/server/test/org/apache/cloudstack/network/lb/CertServiceTest.java \n
cd apache-cloudstack-4.4.2-src/deps \n
...
cd %s/apache-cloudstack-4.4.2-src/dist/rpmbuild/RPMS/x86_64/ \n
createrepo . \n
cp -rf * %s/public/cloudstack.apt-get.eu/rhel/4.4 \n
cd %s \n
#rm -rf apache-* \n
